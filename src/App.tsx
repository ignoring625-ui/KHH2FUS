...
export default function App() {
  ...
  const getUpcomingItems = () => {
    ...
    return future.length > 0 ? future.slice(0, 3) : sorted.slice(-3);
  };
  ...
  const renderContent = () => {
    if (currentView === 'all-itineraries') return <AllItineraries items={itineraryItems} onBack={() => setCurrentView('home')} onUpdate={handleItineraryUpdate} onAdd={handleItineraryAdd} members={members.map(m=>m.name)} />;
