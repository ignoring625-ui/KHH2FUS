...
export interface ItineraryItemData { id: string; date: string; time: string; title: string; location: string; notes?: string; cost?: number; currency?: string; payer?: string; splitters?: string[]; }
...
export const Itinerary: React.FC<ItineraryProps> = ({ items, onUpdate, onAdd, onDelete, onViewAll }) => {
  ...
  const [formData, setFormData] = useState<Partial<ItineraryItemData>>({ date: new Date().toISOString().split('T')[0], time: "12:00 - 13:00", title: "", location: "" });
  ...
  return (
    <section className="px-4">
      ...
      <div className="flex gap-4 overflow-x-auto pb-4 -mx-4 px-4 snap-x no-scrollbar" style={{ WebkitOverflowScrolling: 'touch' }}>
        {items.map((item) => (
        ...
        <button onClick={() => { setEditingItem(null); setFormData({date: new Date().toISOString().split('T')[0], time: "12:00 - 13:00", title: "", location: ""}); setIsAddOpen(true); }} className="flex-shrink-0 w-[120px] border-2 border-dashed border-gray-200 rounded-[28px] flex flex-col items-center justify-center gap-2 text-gray-300"><Plus size={24} /><span className="text-[10px] font-black uppercase">Add</span></button>
      </div>
          ...
          <div className="fixed inset-0 z-[999] flex items-end justify-center">
            ...
            <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} className="relative w-full max-w-md bg-white rounded-t-[40px] p-8 shadow-2xl overflow-y-auto" style={{ maxHeight: '85dvh', paddingBottom: 'max(2rem, env(safe-area-inset-bottom))' }}>
              <div className="flex justify-between items-center mb-8">
                <h3 className="text-2xl font-black uppercase tracking-tighter text-gray-900">{editingItem ? 'Edit' : 'New Trip'}</h3>
              ...
              <div className="flex gap-3">
                {editingItem && <button onClick={() => { if(confirm('確定刪除？')){ onDelete?.(editingItem.id); setIsAddOpen(false); } }} className="flex-1 py-5 bg-red-50 text-red-600 rounded-[20px] font-black uppercase text-xs flex items-center justify-center gap-2"><Trash2 size={18} /> Delete</button>}
                <button onClick={handleSave} className="flex-[2] py-5 bg-black text-white rounded-[20px] font-black uppercase text-xs">Save</button>
              </div>
            </motion.div>
          </div>
        )}
